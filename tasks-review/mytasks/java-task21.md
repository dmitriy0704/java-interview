...
**Причина:** Попытка создать больше потоков, чем позволяет ОС (лимиты `ulimit -u`, память на стеки). Чаще всего вызвана `newCachedThreadPool()` при высокой нагрузке.
...
**Причина:** Неограниченная очередь задач в `FixedThreadPool` или `SingleThreadExecutor`. Если задачи поступают быстрее, чем обрабатываются, очередь растёт и заполняет кучу.
